...
class OrbitNavigator:
    def __init__(self, radius=2, altitude=10, speed=2, iterations=1, center=[1, 0], snapshots=None):
        ...
        self.client.enableApiControl(True, vehicle_name="HALO")

        self.home = self.client.getMultirotorState(vehicle_name="HALO").kinematics_estimated.position
        # check that our home position is stable
        start = time.time()
        count = 0
        while count < 100:
            pos = self.client.getMultirotorState(vehicle_name="HALO").kinematics_estimated.position
    ...
    def start(self):
        global flag
        print("arming the runner...")
        self.client.armDisarm(True, vehicle_name="HALO")

        # AirSim uses NED coordinates so negative axis is up.
        start = self.client.getMultirotorState(vehicle_name="HALO").kinematics_estimated.position
        landed = self.client.getMultirotorState(vehicle_name="HALO").landed_state
        if not self.takeoff and landed == airsim.LandedState.Landed:
            self.takeoff = True
            print("runner taking off...")
            self.client.takeoffAsync(vehicle_name="HALO").join()
            start = self.client.getMultirotorState(vehicle_name="HALO").kinematics_estimated.position
            z = -self.altitude + self.home.z_val
        else:
            print("runner already flying so we will orbit at current altitude {}".format(start.z_val))
            z = start.z_val  # use current altitude then

        print("runner climbing to position: {},{},{}".format(start.x_val, start.y_val, z))
        self.client.moveToPositionAsync(start.x_val, start.y_val, z, self.speed, vehicle_name="HALO").join()
        self.z = z

        print("runner ramping up to speed...")
        count = 0
        ...
        while count < self.iterations:
            ...
            if diff < ramptime:
                ...
                print("runner reached full speed...")
                ramptime = 0
            ...
            if self.track_orbits(angle_to_center * 180 / math.pi):
                count += 1
                print("runner completed {} orbits".format(count))

            self.camera_heading = camera_heading
            self.client.moveByVelocityZAsync(vx, vy, z, 1, airsim.DrivetrainType.MaxDegreeOfFreedom,
                                             airsim.YawMode(False, camera_heading), vehicle_name="HALO")

        self.client.moveToPositionAsync(start.x_val, start.y_val, z, 2, vehicle_name="HALO").join()

        if self.takeoff:
            # if we did the takeoff then also do the landing.
            if z < self.home.z_val:
                print("runner descending")
                self.client.moveToPositionAsync(start.x_val, start.y_val, self.home.z_val - 5, 2,
                                                vehicle_name="HALO").join()

            print("runner landing...")
            self.client.landAsync(vehicle_name="HALO").join()

            print("runner disarming.")
            self.client.armDisarm(False, vehicle_name="HALO")
            self.client.enableApiControl(False, vehicle_name="HALO")
    ...
    def track_orbits(self, angle):
        ...
        if self.snapshot_delta and angle > self.next_snapshot:
            print("runner Taking snapshot at angle {}".format(angle))
            self.take_snapshot()
    ...
    def take_snapshot(self):
        ...
        self.client.moveToPositionAsync(pos.x_val, pos.y_val, self.z, 0.5, 10, airsim.DrivetrainType.MaxDegreeOfFreedom,
                                        airsim.YawMode(False, self.camera_heading), vehicle_name="HALO").join()
    ...
    def state_extractor(self):
        state1 = self.client.simGetObjectPose('HALO')
        s = pprint.pformat(state1)
        print("state HALO (Runner): %s" % s)
    ...
    def sinusoidal_slalom(self, time_thresh = 60):
        print("arming the runner...")
        self.client.armDisarm(True, vehicle_name="HALO")
        ...
        while True:
            ...
            Vz = 0
            theta = theta+0.1
            self.client.moveByVelocityBodyFrameAsync(1, Vy, 2, 0.1,
                                                     yaw_mode=YawMode(is_rate=True, yaw_or_rate=0),
                                                     vehicle_name="HALO").join()

            # self.client.moveToZAsync(z, 1, vehicle_name="HALO").join()
            current_time = time.time()
            if current_time - init_time > time_thresh:
                break
    ...
    def go_straight(self, time_thresh = 60):
        print("arming the runner...")
        self.client.armDisarm(True, vehicle_name="HALO")
